...
class Context_comparison(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size_L, kernel_size_W, stride):
        ...
        self.ca = ChannelAttention(out_ch, 8)
        self.sp = SpatialAttention(kernel_size_L, kernel_size_W, stride=1)
        self.conv1m1 = nn.Conv2d(
            in_channels=out_ch * 3, out_channels=out_ch, kernel_size=(1, 1), padding=0
        )
...
class IMUnet(nn.Module):  # 库中的torch.nn.Module模块
    def __init__(self, in_channels=1, input_length=3600):
        ...
        size_level1 = input_length // 10
        size_level2 = input_length // 5
        size_level3 = input_length // 1

        self.conv1_1 = conv_1_block_DW(
            in_channels, 16, kernel_size_L=1, kernel_size_W=25, stride=1
        )
        self.conv1_2 = conv_1_block_DW(
            16, 16, kernel_size_L=1, kernel_size_W=25, stride=1
        )
        self.conv1_3 = conv_1_block_DW(
            16, 16, kernel_size_L=1, kernel_size_W=25, stride=1
        )

        self.conv2_1 = conv_1_block_DW(
            16, 32, kernel_size_L=1, kernel_size_W=15, stride=1
        )
        self.conv2_2 = conv_1_block_DW(
            32, 32, kernel_size_L=1, kernel_size_W=15, stride=1
        )
        self.conv2_3 = conv_1_block_DW(
            32, 32, kernel_size_L=1, kernel_size_W=15, stride=1
        )

        self.conv3_1 = conv_1_block_DW(
            32, 48, kernel_size_L=1, kernel_size_W=5, stride=1
        )
        self.conv3_2 = conv_1_block_DW(
            48, 48, kernel_size_L=1, kernel_size_W=5, stride=1
        )
        self.conv3_3 = conv_1_block_DW(
            48, 48, kernel_size_L=1, kernel_size_W=5, stride=1
        )

        self.conv4_1 = conv_1_block_MD(
            48, 64, kernel_size_L=1, kernel_size_W=3, stride=1
        )
        #        self.conv4_2=conv_1_block_MD(64, 64, kernel_size_L=1,kernel_size_W=3,stride=1)
        self.conv4_2 = Context_comparison(
            64, 64, kernel_size_L=1, kernel_size_W=3, stride=1
        )
        self.conv4_3 = conv_1_block_MD(
            64, 64, kernel_size_L=1, kernel_size_W=3, stride=1
        )

        self.conv5_1 = conv_1_block_UP(
            48 + 64, 48, kernel_size_L=1, kernel_size_W=5, stride=1
        )
        self.conv5_2 = conv_1_block_UP(
            48, 48, kernel_size_L=1, kernel_size_W=5, stride=1
        )
        self.conv5_3 = conv_1_block_UP(
            48, 32, kernel_size_L=1, kernel_size_W=5, stride=1
        )

        self.conv6_1 = conv_1_block_UP(
            32 + 32, 32, kernel_size_L=1, kernel_size_W=15, stride=1
        )
        self.conv6_2 = conv_1_block_UP(
            32, 32, kernel_size_L=1, kernel_size_W=15, stride=1
        )
        self.conv6_3 = conv_1_block_UP(
            32, 16, kernel_size_L=1, kernel_size_W=15, stride=1
        )

        self.conv7_1 = conv_1_block_UP(
            16 + 16, 16, kernel_size_L=1, kernel_size_W=25, stride=1
        )
        self.conv7_2 = conv_1_block_UP(
            16, 16, kernel_size_L=1, kernel_size_W=25, stride=1
        )
        self.conv7_3 = conv_1_block_UP(
            16, 16, kernel_size_L=1, kernel_size_W=25, stride=1
        )

        self.conv1m1 = nn.Conv2d(
            in_channels=16, out_channels=1, kernel_size=(1, 1), padding=0
        )
        ...
        self.upsample1 = nn.ConvTranspose2d(
            64, 64, kernel_size=(1, 2), stride=(1, 2)
        )  # 2x upsample
        self.upsample2 = nn.ConvTranspose2d(
            32, 32, kernel_size=(1, 2), stride=(1, 2)
        )  # 2x upsample
        self.upsample3 = nn.ConvTranspose2d(
            16, 16, kernel_size=(1, 5), stride=(1, 5)
        )  # 5x upsample
