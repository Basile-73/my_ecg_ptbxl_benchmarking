...
from mycode.classification.utils import utils
from mycode.eda import eda_utils
...
def plot_signal_statistics(stats_df, output_folder):
    ...
    One box per lead showing the distribution across all samples.
    """
    ...
    # Each lead gets its own box showing the distribution of means across all records
    mean_data = [stats_df.loc[i, 'means'] for i in range(len(stats_df))]
    bp1 = axes[0].boxplot(mean_data, labels=stats_df['lead'].tolist(), patch_artist=True,
                          widths=0.6, showmeans=True,
                          boxprops=dict(facecolor='steelblue', alpha=0.7),
                          medianprops=dict(color='red', linewidth=2),
                          meanprops=dict(marker='D', markerfacecolor='green', markersize=8))
    axes[0].set_title('Signal Mean per Lead', fontsize=14, fontweight='bold')
    ...
    # Each lead gets its own box showing the distribution of stds across all records
    std_data = [stats_df.loc[i, 'stds'] for i in range(len(stats_df))]
    bp2 = axes[1].boxplot(std_data, labels=stats_df['lead'].tolist(), patch_artist=True,
                          widths=0.6, showmeans=True,
                          boxprops=dict(facecolor='coral', alpha=0.7),
                          medianprops=dict(color='red', linewidth=2),
                          meanprops=dict(marker='D', markerfacecolor='green', markersize=8))
    axes[1].set_title('Signal Standard Deviation per Lead', fontsize=14, fontweight='bold')
    axes[1].set_ylabel('Std Value', fontsize=12)
    axes[1].set_xlabel('Lead', fontsize=12)
...
def plot_flipped_signal_examples(data, flipped_indices_per_lead, output_folder, n_examples=10):
    ...
    for rec_idx in selected_indices:
        ...
        for lead_idx in range(min(12, signal.shape[1])):
            ...
            try:
                flip_result = eda_utils.detect_flipped_records(raw_signal, SAMPLING_RATE)
...
def main():
    ...
    stats_df, flipped_indices_per_lead = eda_utils.compute_signal_statistics(data, LEAD_NAMES, SAMPLING_RATE)
    ...
    superclass_stats = eda_utils.compute_statistics_by_superclass(data, labels_superdiag, LEAD_NAMES, SAMPLING_RATE)
